...
> 다만 (1) 실제 보험료는 아직 확보하지 못해 순위에서 가격 축을 빼고 나머지 축으로 재정규화하며,
> (2) 실속케어·안심케어 2등급만 판매해 고급 등급 비교에서는 제외되고 그 사유를 화면에 밝힙니다.
...
**현재 데이터 규모**(2026-08-30 기준): 조항(Clause) 558건, 담보(Coverage) 170건, 사고유형 매핑(ClauseIncidentMap) 711건, 수치 조건(ClauseTerm) 174건, 청구서류가 성립하는 담보 163건 전부 필요서류 연결(100%). 원본 재구성 스크립트는 `backend/app/seed_*.py`에 남아 있습니다.

이 숫자를 여기에 적어두는 것만으로는 계속 낡습니다(실제로 보험사가 7개로 늘어난 뒤 위 숫자가 한동안 어긋나 있었습니다). 그래서 앱 안의 「약관 형광펜」 화면 아래쪽에 **근거 검증 현황** 구획을 두고, 지금 이 서버의 DB에서 그 자리에서 세어 보여줍니다 (`GET /insurers/kb-stats`). 통과율이 100%가 아닌 항목도 그대로 내보입니다 — 못 채운 자리를 가리면 채운 자리의 숫자까지 믿을 수 없게 되기 때문입니다.
...
- pytest 도입(188건) — 진단 인용문이 실제 조항 원문의 부분 문자열인지, 안내 문구의 핵심 주장이
...
- **항공지연 통계 — 약관의 지연기준시간을 실제 숫자로 체감** — 약관은 "4시간 이상
  ...
  공공데이터포털에 공개한 실제 출도착 지연 통계(2017~2025.5, 전국공항, 지연 49만여 건)를
  받아, 약관형광펜·청구전점검 화면의 지연기준시간 배지 옆에 "국제선 실제 평균 지연시간은
  약 61분 — 이 기준(240분)은 평균의 약 4배"처럼 크기 비교를 붙입니다. 원본에 총
  운항편수가 없어 **"지연 발생 확률"은 계산하지 않습니다** — 확률을 주장하면 근거 없는
  숫자가 되므로, 크기 비교(평균 대비 몇 배)까지만 씁니다. 새 화면을 만들지 않고 기존
  "약관 형광펜"·"청구 전 점검" 흐름에 이미 있는 조항 카드에 그대로 얹었습니다.

- **부지급 사유 조항 확인 — "약관 형광펜"에서** — 지금까지 생애주기는 "여행 전 →
  ...
  **법률 자문이 아닙니다** — 조항 원문과 매핑 사실까지만 보여주고, 이번 사고에 실제
  ...
  직접 묻거나 금융감독원 분쟁조정(1332)을 안내합니다. 새 화면 대신 "약관 형광펜"에서
...
- **「현지에서」 — 해외 현지에서 쓰는 서류 요청 카드** — 생애주기가 "여행 전 → 사고 접수
  ...
  = `현지only`, 14종 중 9종)를 맨 위로 올리고 여행 종료일 기준 D-day를 함께 보여줍니다.
