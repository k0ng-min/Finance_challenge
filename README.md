...
담보 가입금액표는 7개사 전부 확보돼 있습니다(6개사는 `seed_plan_coverage`, 신한EZ손보는 다이렉트 화면 캡처 근거로 `seed_shinhan_2026_f`).
...
- **사고 시뮬레이션 — "이 여행에서 이런 일이 나면 보험사별로 어떻게 갈리는가"** — 지금까지
  ...
  판정 엔진에 그대로 태워서**, 같은 사고에 6개사 결론이 조항 원문과 함께 갈리는 것을
  보여줍니다. 예를 들어 활동에 "스쿠버다이빙"을 적으면 기존 *활동 기반 면책 우선* 판정
  (`claim_review.rank_maps`)이 그대로 발동해 6개사 전부 면책으로 뒤집히고, 그 근거가 된
  문구가 실제로 담긴 조항 구간이 인용됩니다. 배상책임 시나리오에서는 카카오페이손해보험만
