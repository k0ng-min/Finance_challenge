...
from app.models.user import AppUser, Incident, Evidence
...
def _run_analysis(db: Session, incident: Incident, merged: dict[str, ExtractedField]) -> IncidentAnalysisOut:
    ...
    finding_specs = generate_claim_findings(db, incident.user_id, merged)
...
def answer_question(incident_id: int, payload: AnswerIn, db: Session = Depends(get_db)):
    incident = db.get(Incident, incident_id)
    if not incident:
        raise HTTPException(status_code=404, detail="사고 정보를 찾을 수 없습니다.")
...
def get_checklist(incident_id: int, db: Session = Depends(get_db)):
    incident = db.get(Incident, incident_id)
    if not incident:
        raise HTTPException(status_code=404, detail="사고 정보를 찾을 수 없습니다.")
    ...
    seen = set()
    for uc, cov, insurer in iter_relevant_user_coverages(db, incident.user_id, merged):
...
def submit_evidence(incident_id: int, payload: list[EvidenceIn], db: Session = Depends(get_db)):
    incident = db.get(Incident, incident_id)
    if not incident:
        raise HTTPException(status_code=404, detail="사고 정보를 찾을 수 없습니다.")
    ...
    return get_checklist(incident_id, db)
