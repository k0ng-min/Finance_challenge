...
5. "자료 없음"인 축을 0점이나 좋은 점수로 추정하지 말고, 보험사 간 비교 근거로 쓰지 마세요.
   그 축이 왜 빠졌는지는 말해도 되지만, 그것으로 우열을 가리지는 마세요.
...
def _axis_lines(axes: list[dict]) -> list[str]:
    ...
    자료 없는 축(comparison_state != AVAILABLE)은 목록에서 지우지 않고 "자료 없음"으로
    이름만 남긴다. 점수를 매기는 프롬프트였다면 지우는 게 맞다 — 빠진 축이 비교 근거로
    쓰이면 안 되니까. 하지만 여기는 설명을 쓰는 자리다. 이름이 없으면 모델은 그 축이
    존재했다는 사실 자체를 모르고, 그러면 "그 축은 자료가 없어 뺐어요"라는 말을 할 수가
    없다. 그건 이 서비스가 사용자에게 꼭 해야 하는 말이다. 대신 프롬프트 규칙 5번이
    그 축을 우열의 근거로 쓰지 못하게 막는다."""
    lines = []
