...
_FALLBACK_KEYWORDS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    # "파손"이라고 적는 사람은 드물다 — 실제로는 "망가졌어요", "고장났어요",
    # "액정에 금이 갔어요"라고 쓴다. 이걸 못 잡으면 대분류가 SPC로 떨어져서
    # 반려동물·전쟁 같은 엉뚱한 질문이 나오고 관련 약관도 하나도 안 걸린다.
    ("PROP", ("도난", "소매치기", "분실", "잃어버", "훔쳐", "여권",
              "파손", "깨졌", "깨짐", "망가졌", "망가짐", "고장", "부서졌", "부서짐",
              "찌그러", "금이 갔", "액정", "침수", "물에 빠")),
...
_FALLBACK_L2_KEYWORDS: dict[str, tuple[tuple[str, tuple[str, ...]], ...]] = {
    "PROP": (
        ...
        ("PROP_DAMAGE", ("파손", "깨졌", "깨짐", "깨뜨", "부서", "망가", "고장",
                         "찌그러", "금이 갔", "액정", "침수")),
