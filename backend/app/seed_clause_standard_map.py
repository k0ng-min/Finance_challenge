...
- 제3조(해외 상해의료비, "보험가입금액을 한도로 피보험자가 실제 부담한 의료비 전액을
  보상합니다"): 삼성·메리츠·KB·DB·카카오페이 5개사에서 확인. 현대는 실손의료비를
  상해/질병으로 나누지 않고 "기본형 해외여행 급여 실손의료비보장" 하나로 묶어서 다뤄
  (OVS_INJ_MED가 아니라 OVS_MED_BASIC 담보) 이 문구 자체가 없다 — 근거가 없어 만들지
  않는다(단정하지 않는다).
- 제4조(전쟁·내란 면책, "전쟁, 외국의 무력행사, 혁명, 내란, 사변, 폭동으로 인한 경우"):
  삼성·메리츠·KB·DB 4개사에서 확인. 현대·카카오페이는 실손의료비 특약 자체에 이 문구를
  반복하지 않아(상해사망 보통약관에서만 규정하거나 다른 표현을 씀) 근거 조항을 못
  찾았다 — 마찬가지로 단정하지 않는다.
