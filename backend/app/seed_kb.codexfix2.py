"""
KB손해보험 해외여행보험(15332, 2020-04 배포본) 실물 약관 KB 시드 스크립트.

원문 출처: data/raw_pdfs/kb_overseas_15332_202004.pdf
(docs/compliance/source_register.md 참조)

주의: 2026-07-28 KB손해보험이 '개정 출시'한 신규 해외여행보험(고급형/표준형/실속형) 관련
뉴스가 확인되었으나, 개정판 약관 PDF의 정적 다운로드 링크는 찾지 못했다(페이지가 JS로
동적 렌더링됨). 따라서 이번 시드는 2020-04 배포본 기준이며, review_status=raw로 표시하고
"최신 버전 재확인 필요"를 source_register.md에 남긴다.

삼성/현대와 동일한 표준담보 3종을 매핑한다. KB는 구조송환비용 입원 요건이 14일로
고정되어 있고(삼성은 14/7/4일 중 선택), 면책 범위에 전쟁 조항(제5호)까지 포함하는 등
회사별 실제 문구 차이가 있다.
"""
...
def run():
    ...
    try:
        insurer, product, pv = seed_insurer_core(
            db,
            name="KB손해보험", code="KB", official_url="https://www.kbinsure.co.kr",
            product_name="KB해외여행보험", product_code="15332", channel="다이렉트",
        ...
        cov_death_injury = Coverage(
            ...
            definition=(
                "회사는 피보험자가 보험증권에 기재된 해외여행을 목적으로 주거지를 출발하여 여행을 마치고 "
                "주거지에 도착할 때까지의 여행 도중에 다음 중 어느 하나의 사유가 발생한 경우에는 보험수익자에게 "
        ...
        cov_rescue = Coverage(
            ...
            definition=(
                "회사는 여행 도중 피보험자가 탑승한 항공기·선박의 행방불명·조난, 산악등반 중 조난, "
                "긴급수색구조가 필요한 사고(경찰 등 공공기관 확인), 상해를 직접원인으로 사고일부터 1년 이내 "
                "사망 또는 14일 이상 계속 입원, 질병을 직접원인으로 한 사망·14일 이상 입원 시 수색구조비용·"
            ...
            waiting_condition="입원의 경우 14일 이상 계속 입원 필요(삼성 등 타사와 달리 일수 선택 옵션 없음)",
        )
        ...
        clauses = [
            Clause(
                ...
                text=(
                    ...
                    "약정한 보험금을 지급합니다. 1. 보험기간 중에 상해의 직접결과로써 사망한 경우(질병으로 인한 "
                    "사망은 제외합니다): 사망보험금 2. 보험기간 중 상해로 장해분류표(<별표1> 참조)에서 정한 각 "
                    "장해지급률에 해당하는 장해상태가 되었을 때: 후유장해보험금(장해분류표에서 정한 지급률을 "
                    "보험가입금액에 곱하여 산출한 금액)"
                ),
                page_ref="p.19", embedding_id=None, default_color="파랑",
            ),
            Clause(
                policy_version_id=pv.policy_version_id, coverage_id=cov_death_injury.coverage_id,
            ...
            Clause(
                ...
                text=(
                    ...
                    "제출하는 서류(사망보험금 지급시 피보험자의 법정상속인이 아닌자가 청구하는 경우 법정상속인의 "
                    "확인서) ② 사고증명서는 의료법 제3조(의료기관)에서 규정한 국내의 병원이나 의원 또는 국외의 "
                    "의료관련법에서 정한 의료기관에서 발급한 것이어야 합니다."
                ),
                page_ref="p.21", embedding_id=None, default_color="노랑",
            ),
            Clause(
                ...
                page_ref="p.97-98", embedding_id=None, default_color="파랑",
            ),
            Clause(
                policy_version_id=pv.policy_version_id, coverage_id=cov_rescue.coverage_id,
                clause_type="보장정의", article_no="제1조(보험금의 지급사유)~제2조(비용의 범위)",
                text=(
                    "① 회사는 여행 도중에 피보험자가 탑승한 항공기 또는 선박이 행방불명 또는 조난된 경우, "
                    "산악등반 중 조난된 경우, 급격하고도 우연한 외래의 사고로 긴급수색구조 등이 필요한 상태로 된 것이 "
                    "경찰 등 공공기관에 의해 확인된 경우, 보통약관 제3조의 상해를 직접원인으로 사고일로부터 1년 "
                    "이내 사망하거나 14일 이상 계속 입원한 경우, 질병을 직접원인으로 여행 도중 사망 또는 14일 이상 "
                    "입원한 경우에 비용을 보상합니다. 비용의 범위: 1.수색구조비용 2.항공운임 등 교통비(구원자 "
                    "2명분 한도) 3.숙박비(구원자 2명분, 1인당 14박분 한도) 4.이송비용 5.제잡비(10만원 한도)"
                ),
                page_ref="p.65", embedding_id=None, default_color="파랑",
            ),
            Clause(
                policy_version_id=pv.policy_version_id, coverage_id=cov_rescue.coverage_id,
                clause_type="면책", article_no="제3조(보험금을 지급하지 않는 사유)",
                text=(
                    "회사는 보통약관 제5조(보험금을 지급하지 않는 사유) 제1항 제1호 내지 제3호, 제5호의 사유로 "
                    "인하여 생긴 손해는 보상하여 드리지 않습니다. (제5호=전쟁·무력행사·혁명·내란·사변·폭동까지 "
                    "명시 — 삼성화재는 제1~3호만 배제하여 KB보다 면책범위가 좁음)"
                ),
                page_ref="p.65-66", embedding_id=None, default_color="빨강",
            ),
        ]
        db.add_all(clauses)
        db.flush()
        ...
        db.commit()
        print(f"KB손해보험 KB 시드 완료: coverage=3, clause={len(clauses)}, coverage_doc_map={len(doc_maps)}")
    finally:
        db.close()
