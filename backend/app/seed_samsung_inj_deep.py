"""
삼성화재(기준 보험사) INJ 상해 딥다이브 1차분 — data/raw_pdfs/samsung_overseas_50002_0_20240401.pdf
직접 페이지를 읽고 원문을 대조해 확인한 두 가지를 반영한다:

1. clause_id=2(DEATH_INJURY 면책, 제5조)가 PDF p.13~14에서 잘린 채로 시딩돼 있었다.
   ②항의 "직업·직무·동호회 활동목적" 면책 사유(전문등반/스카이다이빙/스쿠버다이빙/
   오토바이 경기 등 — incident.modifiers.activity와 직결되는 부분)가 통째로 빠져 있어서,
   원문 전체로 갱신한다.
2. INJ_DOMESTIC_TREATMENT(귀국후 국내치료)에 매핑된 조항이 하나도 없었다. 실제로는
   "기본형 해외여행 실손의료비 특별약관" 제3조 (1)상해 "국내(급여)" 항목(p.57)이 정확히
   이 유형이다 — 해외여행 중 입은 상해로 귀국 후 국내에서 치료받는 경우를 다룬다.
   기존 OVS_INJ_MED 담보(coverage_id 재사용, 같은 특약의 하위 항목)에 조항만 추가한다.

이후 KB/DB/현대/메리츠/카카오페이도 같은 방식(PDF 직접 대조 후 갱신·추가)으로 이어간다.
"""
...
DEATH_INJURY_WAIVER_FULL = (
    ...
    "1. 피보험자가 고의로 자신을 해친 경우. 다만, 피보험자가 심신상실 등으로 자유로운 의 "
    "사결정을 할 수 없는 상태에서 자신을 해친 경우에는 보험금을 지급합니다. "
    "2. 보험수익자가 고의로 피보험자를 해친 경우. 다만, 그 보험수익자가 보험금의 일부 보"
    "험수익자인 경우에는 다른 보험수익자에 대한 보험금은 지급합니다. "
    "3. 계약자가 고의로 피보험자를 해친 경우 "
    "4. 피보험자의 임신, 출산(제왕절개를 포함합니다), 산후기. 그러나, 회사가 보장하는 보험"
    "금 지급사유와 보장개시일부터 2년이 지난 후에 발생한 습관성 유산, 불임 및 인공수정 "
    "관련 합병증으로 인한 경우에는 보험금을 지급합니다. "
    "5. 전쟁, 외국의 무력행사, 혁명, 내란, 사변, 폭동 "
    "② 회사는 다른 약정이 없으면 피보험자가 직업, 직무 또는 동호회 활동목적으로 아래에 열거 "
    "된 행위로 인하여 제3조(보험금의 지급사유)의 상해 관련 보험금 지급사유가 발생한 때에"
    "는 해당 보험금을 지급하지 않습니다. "
    "1. 전문등반(전문적인 등산용구를 사용하여 암벽 또는 빙벽을 오르내리거나 특수한 기술, "
    "경험, 사전훈련을 필요로 하는 등반을 말합니다), 글라이더 조종, 스카이다이빙, 스쿠버"
    "다이빙, 행글라이딩, 수상보트, 패러글라이딩 "
...
DOMESTIC_TREATMENT_TEXT = (
    "① 회사는 피보험자가 보험증권에 기재된 해외여행 중에 상해를 입고, 이로 인해 국내 의료"
    "기관ㆍ약국에서 치료를 받은 때에는 붙임2에 따라 보상합니다. 다만, 보험기간이 1년 미만인 "
    "경우에는 해외여행 중에 피보험자가 입은 상해로 보험기간 종료후 30일(보험기간 종료일은 "
    "제외합니다) 이내에 의사의 치료를 받기 시작했을 때에는 의사의 치료를 받기 시작한 날부터 "
    "180일(통원은 180일 동안 90회)까지만(보험기간 종료일은 제외합니다) 보상합니다."
)
...
def run():
    ...
    try:
        ...
        db.commit()
        print(f"samsung INJ 딥다이브 1차: 면책조항 보정={updated}, 국내치료 조항 추가={inserted}")
    finally:
        db.close()
