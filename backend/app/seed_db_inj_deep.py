"""
DB손해보험 INJ 상해 딥다이브 1차분 — data/raw_pdfs/db_overseas.pdf (프로미 해외여행보험Ⅰ)
삼성화재(seed_samsung_inj_deep.py)와 같은 방식으로 PDF 원문을 직접 대조해 확인한 두 가지를
반영한다:

1. clause_id=30(상해사망·후유장해 면책, 제5조)가 PDF p.5에서 "...아래에 열거된 행위로 인하여"
   에서 잘린 채로 시딩돼 있었다. ②항의 "직업·직무·동호회 활동목적" 면책 사유(전문등반/글라이더
   조종/스카이다이빙/스쿠버다이빙/행글라이딩/수상보트/패러글라이딩, 모터보트·자동차·오토바이 경기,
   선박승무원 등 — incident.modifiers.activity와 직결되는 부분)가 통째로 빠져 있어서 원문 전체로
   갱신한다. (참고: 기존에 저장돼 있던 "폭동② 회사는" 사이의 누락된 공백도 자연스러운 한국어 표기에
   맞춰 "폭동 ② 회사는"으로 함께 바로잡는다 — PDF 원문 자체는 그대로이며 띄어쓰기만 보정.)
2. INJ_DOMESTIC_TREATMENT(귀국후 국내치료)에 매핑된 조항이 하나도 없었다. 기존에는 "기본형 실손
   의료비 특별약관" 제3조 (1)상해의료비 항목 중 "해외" 세부항목(coverage_id=14, clause_id=32)만
   시딩돼 있었고, 바로 옆의 "국내" 세부항목(PDF p.21, 표에서 "국내" 행)이 빠져 있었다. 실제로는
   해외여행 중 입은 상해로 귀국 후 국내 의료기관·약국에서 치료받는 경우를 다룬다 — 이 유형이 정확히
   INJ_DOMESTIC_TREATMENT다. 기존 coverage_id=14(같은 특약의 하위 항목)에 조항만 추가한다.

원문은 pdfplumber extract_text()로 페이지를 라인 단위로 추출한 뒤, 표 라벨("국내", "상해", "의료비"
등 셀 헤더가 본문 문장 사이에 끼어드는 부분)만 제거하고, 줄바꿈으로 끊긴 지점의 원래 띄어쓰기를
문법에 맞게 복원해 재구성했다(의역·재구성 없음, 각 문장은 PDF 원문 그대로).

이후 KB/현대/메리츠/카카오페이도 같은 방식(PDF 직접 대조 후 갱신·추가)으로 이어간다.
"""
...
DEATH_INJURY_WAIVER_FULL = (
    ...
    "인하여 제3조(보험금의 지급사유)의 상해 관련 보험금 지급사유가 발생한 때에는 해당 보험금을 지급하지 "
    "않습니다. "
...
DOMESTIC_TREATMENT_TEXT = (
    ...
    "약국에서 치료를 받은 때에는 <붙임2>에 따라 보상합니다. 다만, 보험기간이 1년 미만인 경우에는 "
...
def run():
    ...
    try:
        ...
        # 1) 상해사망·후유장해 면책 조항 원문 보정 (잘린 채로 시딩돼 있던 clause 갱신)
        waiver = (
            db.query(Clause)
        ...
        # 2) 귀국후 국내치료(INJ_DOMESTIC_TREATMENT) 조항 추가 — 기존 실손의료비 특약(상해 해외의료비)
        # coverage_id에 "국내(급여)" 세부항목만 새 조항으로 얹는다(같은 특약의 하위 항목이라 재사용).
        ...
        if ovs_inj_med_coverage:
            ...
            if not exists:
                new_clause = Clause(
                    ...
                    clause_type="보장정의",
                    article_no="제3조(보장종목별 보상내용) (1)상해의료비-국내",
        ...
        db.commit()
        print(f"DB손보 INJ 딥다이브 1차: 면책조항 보정={updated}, 국내치료 조항 추가={inserted}")
    finally:
        db.close()
