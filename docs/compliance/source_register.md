...
- **`ClauseStandardMap`(표준약관 대조, 9건)**: 제3조(해외 상해의료비)는 삼성·메리츠·
  KB·DB·카카오페이 5개사에서, 제4조(전쟁·내란 면책)는 삼성·메리츠·KB·DB 4개사에서
  근거를 확인했다. 현대는 실손의료비를 상해/질병으로 나누지 않고 하나로 묶어 다뤄
  (`OVS_MED_BASIC`) 제3조 대응 문구 자체가 없고, 카카오페이는 제4조 문구를 실손
  특약에 반복하지 않는다 — 둘 다 단정하지 않고 행을 만들지 않았다.

## 알려진 남은 공백

- 실손 표준약관이 이 KB에 없어, 여행자보험의 해외 의료비 담보가 기존 실손과 정확히
  얼마나 겹치는지는 여전히 확인불가(`UNKNOWN`)로 남아 있다.
- 현대해상의 실손의료비 특약(`OVS_MED_BASIC`)은 다른 5개사처럼 상해/질병으로 분리돼
  있지 않다 — 사고유형 매핑과 표준약관 대조에서 현대만 빠지는 원인이다. 나눠 다시
  시드하면 해소된다.
