...
아래 표는 `backend/scripts/generate_docs.py`가 `dataset_manifest.json`에서 만들어 넣는다 —
손으로 고치지 말 것. 예전에는 이 표의 숫자를 사람이 옮겨 적었고, 그래서 보험사 여섯 곳의
`term_count`가 실제로는 12~67건인데 표에는 전부 0으로 남아 있었다. 개수를 바꾸려면
`cd backend && python -m app.kb_manifest --confirm`으로 매니페스트를 DB에 맞춘 뒤 이
스크립트를 돌린다.
...
|---|---|---|---|---|---|---|---|---|---:|---:|---:|---:|---:|---|
| SAMSUNG | 해외여행보험 | 2026수집본 | 미확정 | (공개 URL 없음) | OFFICIAL_ISSUED_FILE | 2026-08-18 | `95fc1ac011933d8b329eeee3698cfd8adffa0540bf353b0e90dca446156406b7` | VERIFIED_ISSUED_FILE | 18 | 52 | 80 | 32 | 69 | 표지가 이미지라 약관 버전 코드와 시행일이 인쇄돼 있지 않다. clause_term(정량조건)·doc_requirement(서류 세부요건)·overlap_rule(중복판정)·clause_standard_map(표준약관 대조)을 이번에 새로 만들었다(term_count 갱신, doc_requirement 1건, overlap_rule 7건, clause_standard_map 11건 — 전부 전역 공용 데이터라 특정 보험사에 종속되지 않는다). |
| HYUNDAI | 다이렉트 해외여행보험 | 8403-0000-20260606 | 2026-06-06 | (공개 URL 없음) | OFFICIAL_ISSUED_FILE | 2026-08-18 | `19fcd2966b4d17fdca41bd8edfdfff028a4de3ba38ed3574e83406262ec3c878` | VERIFIED_ISSUED_FILE | 21 | 57 | 50 | 12 | 72 |  clause_term(정량조건)·doc_requirement(서류 세부요건)·overlap_rule(중복판정)·clause_standard_map(표준약관 대조)을 이번에 새로 만들었다(term_count 갱신, doc_requirement 1건, overlap_rule 7건, clause_standard_map 11건 — 전부 전역 공용 데이터라 특정 보험사에 종속되지 않는다). |
| MERITZ | 다이렉트 해외여행보험 | 메리츠일반-특종/상해/여행B-10-2607A | 미확정 | (공개 URL 없음) | OFFICIAL_ISSUED_FILE | 2026-08-18 | `f58406b6496f1031da5f5c870e73c65b7b85ef06d16e2b04448b53d21b61dbdd` | VERIFIED_ISSUED_FILE | 42 | 143 | 152 | 67 | 162 | 약관번호 뒷자리(2607A)를 시행일로 단정할 근거가 없어 effective_date를 비워 뒀다. clause_term(정량조건)·doc_requirement(서류 세부요건)·overlap_rule(중복판정)·clause_standard_map(표준약관 대조)을 이번에 새로 만들었다(term_count 갱신, doc_requirement 1건, overlap_rule 7건, clause_standard_map 11건 — 전부 전역 공용 데이터라 특정 보험사에 종속되지 않는다). |
| KB | KB해외여행보험(다이렉트) | 일반26-15505-1 | 미확정 | (공개 URL 없음) | OFFICIAL_ISSUED_FILE | 2026-08-18 | `49a5ccecdb5cffe5bcf730efedf13322e0719cff8261aebcf47b724cc064e573` | VERIFIED_ISSUED_FILE | 25 | 57 | 76 | 31 | 92 | 표지 표기는 "2026.06"뿐이라 정확한 시행일로 단정 못해 effective_date를 비워 뒀다. 실손의료비 특별약관(p.170-286)은 핵심 보장정의·면책만 넣고 비급여 세부·미가입자 추가특약은 범위 밖으로 남겼다. clause_term(정량조건)·doc_requirement(서류 세부요건)·overlap_rule(중복판정)·clause_standard_map(표준약관 대조)을 이번에 새로 만들었다(term_count 갱신, doc_requirement 1건, overlap_rule 7건, clause_standard_map 11건 — 전부 전역 공용 데이터라 특정 보험사에 종속되지 않는다). |
| DB | 프로미 해외여행보험Ⅰ | 프로미Ⅰ_2026수집본 | 미확정 | (공개 URL 없음) | OFFICIAL_ISSUED_FILE | 2026-08-18 | `151c57ec603cad5b5d5dcc4128468e2ac9102d45d842554f1d98fbbecbf3e008` | VERIFIED_ISSUED_FILE | 17 | 94 | 171 | 18 | 68 | 버전 코드·시행일이 본문에 인쇄돼 있지 않아 effective_date를 비워 뒀다. clause_term(정량조건)·doc_requirement(서류 세부요건)·overlap_rule(중복판정)·clause_standard_map(표준약관 대조)을 이번에 새로 만들었다(term_count 갱신, doc_requirement 1건, overlap_rule 7건, clause_standard_map 11건 — 전부 전역 공용 데이터라 특정 보험사에 종속되지 않는다). |
| KAKAOPAY | 해외여행보험 | 제2026-0199호 | 2026-05-04 | (공개 URL 없음) | OFFICIAL_ISSUED_FILE | 2026-08-18 | `3ed801afa9c8e930b01aed97f4ba8d67823d9ee7d5e4d2d533a3a80f67b4009c` | VERIFIED_ISSUED_FILE | 18 | 38 | 39 | 14 | 71 | 「함께하는 해외여행보험」(f558fafbd3439e91525bea49cb46299d43afcf667deb20c5cf505edde997d16b)과 「함께하는 해외여행보험II」(a4b41ae4d606121188a838e6ebc7838dc579d6324c433253b7c394738866d19a) 두 파일을 함께 읽어 본문이 「해외여행보험」(기준 파일, 위 sha256)과 동일함을 확인하고 하나로 통합했다. file_hash는 기준 파일의 것이다. clause_term(정량조건)·doc_requirement(서류 세부요건)·overlap_rule(중복판정)·clause_standard_map(표준약관 대조)을 이번에 새로 만들었다(term_count 갱신, doc_requirement 1건, overlap_rule 7건, clause_standard_map 11건 — 전부 전역 공용 데이터라 특정 보험사에 종속되지 않는다). |
| SHINHAN | 신한 SOL 처음해외여행보험 | SOL처음해외여행_20260606 | 2026-06-06 | https://www.shinhanez.co.kr/static/pub/PUB2000T021.html | OFFICIAL_ISSUED_FILE | 2026-08-24 | `b3279327a710acb875693eda0c951e9a13ec2f168f7f202db12057015f77f016` | VERIFIED_ISSUED_FILE | 29 | 117 | 143 | 0 | 99 | 7번째 보험사로 2026-08-25에 추가. 공시 페이지가 화면에 파일 링크를 그리지 않고 자체 API(POST /shezApi)로 내려주는 구조라 source_url은 공시 페이지 주소를 적었다(직접 링크 없음). 약관 원문 236쪽은 전부 정독해 조항을 뽑았고 원문 대조 100%를 통과한다. 실제 보험료는 2026-08-25에 확보했다(다이렉트 조회 244행). 다만 (1) 실속케어·안심케어 2등급만 판매해 고급 등급 비교에서는 제외된다, (2) 등급별 보장금액의 출처가 엑셀이 아니라 다이렉트 「보장 비교하기」 화면 캡처다, (3) clause_term(정량조건)은 아직 만들지 않았다. |
